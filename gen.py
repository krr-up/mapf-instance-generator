...
#		- test if instance is SAT with mif encoding and enabling via argument
...
if args.maze:
	...
	instance_unfilled = getoutput('clingo maze.lp -c w=' + args.size + ' --rand-freq=1 -V0 --out-atomf=%s. --out-ifs="\n" | head -n -1') 
...
if args.random:
	...
	instance_unfilled = getoutput('clingo random.lp -c w=' + args.size + ' -c v=' + numVertices + ' --rand-freq=1 -V0 --out-atomf=%s. --out-ifs="\n" | head -n -1')
...
instance_filled = getoutput('clingo ' + instanceFileName + ' task2.lp -c r=' + args.agents + ' --rand-freq=1 -V0 --out-atomf=%s. --out-ifs="\n" | head -n -1')
...
if args.visualize: os.system('clingo ' + instanceFileName + ' mif_to_asprilo.lp | viz')
